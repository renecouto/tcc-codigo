...
void sendData(uint64_t currentTime) {
  char *dataPoints = (char *) malloc(80*BUFFER_SIZE*sizeof(char));
  uint32_t written = formatDataPoints3(dataPoints, buf,BUFFER_SIZE, currentTime, POLL_DATA_INTERVAL_MS);
  // Serial.println(dataPoints.substring(0, 100));
  ...
  if(WiFi.status() == WL_CONNECTED || (wifiMulti.run(10000) == WL_CONNECTED)) {
    ...
    http.begin(DATA_URL);
    http.addHeader(F("Authorization"), AUTH_TOKEN);
    ...
    Serial.println("could never send data");
  }
  free(dataPoints);
